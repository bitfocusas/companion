...
class BitfocusCloud {
	...
	async connect() {
		...
		setTimeout(async () => {
			try {
				console.log("Result of remote command: ", JSON.stringify(await this.clientCommand(this.clientId, 'getBanks')).substr(0,100));
				console.log("Result of remote command: ", JSON.stringify(await this.clientCommand(this.clientId, 'doesntExist')).substr(0,100));
			} catch (e) {
