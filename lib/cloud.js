...
class Cloud {
	 ...
	 */
	/**
	 * Timer should be cleared when a successfull refresh is done
	 * @type {Interval}
	 * @access protected
	 */
	...
	constructor(system) {
		...
		this.initialRefreshTimer = setInterval(() => this.handleCloudRefresh(), 10000);
	...
	async handleCloudInfrastructure() {
		...
		try {
			...
			if (result.cloud && result.cloud.regions) {
				const regions = [ ...result.cloud.regions, { id: 'local', hostname: '127.0.0.1:8001', label: 'localhost', location: 'us' } ];
					...
					if (Cloud.Regions) {
						for (const id in Cloud.Regions) {
							...
							if (this.regions[id] !== undefined) {
								...
								this.regions[id] = new Region(this, id, Cloud.Regions[id])
							}
						}
					}
				} catch (e) {
				...
				this.setState({ regions: newRegions, authenticating: false })
				this.readConnections(this.data.connections)
			}
		} catch (e) {
			this.debug('Cloud infrastructure error: ', e)
			this.setState({ authenticating: false, error: 'Error fetching region server list' })
		}
	}
	...
	async handleCloudLogin(client, email, password) {
		...
		try {
			...
			this.setState({
				authenticated: false,
				authenticating: false,
				error: 'Cannot reach authentication/cloud-api server: ' + e.response,
			})
			this.destroy()
			return
		}
	...
	async handleCloudRefresh(token) {
		...
		// For transparency, show the user that we're refreshing
		this.setState({ authenticating: true, error: '' })
		try {
			...
			if (response.status === 401) {
				this.setState({
					authenticated: false,
					authenticating: false,
					error: 'Login has expired, please login again.',
				})
				clearInterval(this.initialRefreshTimer);
				return;
			}
			const result = await response.json()
			...
			this.setState({
				authenticated: false,
				authenticating: false,
				error: 'Cannot reach cloud servers, retrying in 10 seconds.',
			})
			this.destroy()
			return
		}
	...
	handleCloudRegionStateSet(client, region, newState) {
		...
		let enableChanges = false;
		if (this.regions[region] !== undefined) {
			if (this.regions[region].state.enabled !== newState.enabled) {
				enableChanges = true;
			}
		...
		if (enableChanges) {
			console.log("NEW:::!:!:!:!:  Update regions to clients");
			const regions = Object.values(this.regions).filter(r => r.state.enabled).map(region => ({ id: region.id, host: region.host }))
			for (let region in this.regions) {
				this.regions[region].updateRegions(regions)
			}
		}
	}
	...
	initializeIO(io) {
		...
		this.io = io
        this.system.on('io_connect', (client) => this.clientConnect(client));
	}
	...
	setState(draftState) {
		...
		if (!isEqual(newState, this.state)) {
			this.io.httpIO.emit('cloud_state', newState)
			this.state = newState
		}
			...
			for (let region in this.regions) {
				...
				if (this.state.authenticated !== newState.authenticated) {
					if (currentRegion.state?.enabled && newState.authenticated) {
						...
						currentRegion.setState({ enabled: false })
						currentRegion.setState({ enabled: true })
					} else {
						this.state = newState
	...
	async updateBank(page, bank, style) {
		...
		if (isEqual(this.banks[page][bank], style)) {
			this.system.emit('log','cloud','debug', 'Ignoring bank update since nothing changed: ' + page + '.'+ bank)
			return;
		}

		this.banks[page][bank] = cloneDeep(style)
...
class Region {
	...
	async clientGetBanks(args) {
		this.debug('Client requested getBanks()')
	...
	async clientPushBank(args) {
		this.debug('Client requested pushBank(' + JSON.stringify(args) + ')')
	...
	async clientReleaseBank(args) {
		this.debug('Client requested releaseBank(' + JSON.stringify(args) + ')')
		if (args.bank && args.page) {
	 ...
	 * @access protected
	 */
	async clientPing(args) {
		this.debug('Client requested ping()')
		return true
	}
	...
	async cloudConnect() {
		...
		console.log("REGION " + this.id + " CONNECTING, state: ", this.state.enabled)
		...
		;(async () => {
			while (this.socket && this.state.enabled) {
				for await (let _event of this.socket.listener('connect')) {
					...
					// TODO: Retry login if it fails of timeout etc
					try {
				...
				await delay(1000)
				this.debug('Are we still connected?')
			}
		})()
		;(async () => {
			while (this.socket && this.state.enabled) {
				for await (let event of this.socket.listener('authenticate')) {
					...
					this.transmitFull()
					this.system.emit('log', 'cloud', 'info', 'Connected to cloud')
				}
			}
		})()
		;(async () => {
			while (this.socket && this.state.enabled) {
				for await (let event of this.socket.listener('authStateChange')) {
					// TODO: Remove when disconnected
					if (event.oldAuthState === this.socket.AUTHENTICATED && event.newAuthState === this.socket.UNAUTHENTICATED) {
						try {
							const login = await this.socket.invoke('cloudLogin', {
								token: this.cloud.data.token,
								uuid: this.cloud.state.uuid,
							...
							this.system.emit('log', 'cloud', 'info', 'Reauthenticating to cloud: ' + JSON.stringify(login));
						} catch (e) {
							this.system.emit('log', 'cloud', 'error', 'Error reauthenticating to cloud: ' + e.message);
						}
					}
				}
			}
	...
	/**
	 * Update clients with what regions we are connecting to
	 */
	async updateRegions(regions) {
		console.log("UPDATE REGIONS::::" + this.id, regions);
		if (this.socket && this.state.enabled) {
			this.socket.transmitPublish('companion-regions:' + this.cloud.state.uuid, regions);
		}
	}
	...
	destroy() {
		...
		if (this.state.enabled) {
			// This will reinvoke this function
			this.setState({
				enabled: false,
                error: ''
			});
	...
	registerCompanionProcs(socket, name, callback) {
		if (typeof callback === 'function') {
			;(async () => {
				// Reinstate listener if abruptly finished while we are still enabled
	...
	setState(draftState) {
		...
		if (!isEqual(newState, this.state)) {
			this.io.httpIO.emit('cloud_region_state', this.id, newState)
		}
		...
		if (this.cloud.data.token) {
			if (this.state.enabled !== newState.enabled) {
				if (newState.enabled && this.cloud.state.authenticated) {
					...
					console.log("Region: " + this.id + ": Aborting state change")
					abortState = true
				}
			...
			if (this.state.enabled !== newState.enabled && !newState.enabled) {
				this.destroy()
			}
		}
		...
		if (this.socket !== undefined) {
			;(async () => {
				try {
					...
					this.debug(`couldn't ping ${this.state.name}: %o`, e.message)
				}
			})()
		}
	}
