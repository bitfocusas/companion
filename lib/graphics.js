...
graphics.prototype.drawBankImage = function (c, page, bank) {
	...
	if (c.style == 'pageup') {
		self.lastdrawproperties[page + '_' + bank] = 'pageup'
		...
		self.lastdrawproperties[page + '_' + bank] = 'pagedown'
		...
		self.lastdrawproperties[page + '_' + bank] = 'pagenum'
		...
		let removeTopBar = c.show_topbar === 'default' || c.show_topbar === undefined ? self.remove_topbar : !c.show_topbar

		// Background
		removeTopBar ? img.boxFilled(0, 0, 71, 71, c.bgcolor) : img.boxFilled(0, 14, 71, 71, c.bgcolor)
		...
		if (c.png64 !== undefined && c.png64 !== null) {
			try {
				...
				self.drawTopbar(img, c, removeTopBar, page, bank)
				return img
			}
		}
...
graphics.prototype.drawTopbar = function (img, c, removeTopBar, page, bank) {
	var self = this
...
graphics.prototype.drawBank = function (page, bank) {
	...
	if (
		...
		self.system.emit('variable_parse', c.text, function (str) {
			c.text = str
		})

		img = self.drawBankImage(c, page, bank)
		self.system.emit('graphics_bank_redrawn', page, bank, c)
	} else {
		img = self.buffers[page + '_' + bank] = new Image(72, 72)
